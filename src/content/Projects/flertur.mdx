---
title: "FlerTur"
date: "2026-01-22"
svg: |
  <svg width="80px" height="80px" viewBox="0 -15.5 1055 1055" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="currentColor">
    <path d="M219.508059 593.933479h600.814167s-23.510324 70.530972-62.694197 70.530972H229.946643l-10.438584-70.530972zM222.109868 348.391655h130.62336v143.663753zM459.830591 356.22843h143.679427l151.500528 122.770912h-148.898719z" />
    <path d="M751.421303 677.536192H224.727351a15.673549 15.673549 0 0 1-15.67355-15.67355V277.89203a15.673549 15.673549 0 0 1 31.347099 0v368.297063h511.020403c22.115378 0 41.5976-12.726922 53.446803-34.920668 14.451013-27.06822 13.353864-60.546921-2.664503-85.938071L576.002939 338.580013a15.01526 15.01526 0 0 1-1.285231-1.175516c-27.397364-28.275083-62.584483-43.838918-99.072506-43.838918H300.493288a15.673549 15.673549 0 0 1 0-31.347098h175.151914c44.763657 0 87.693509 18.714218 120.968454 52.741493l227.407528 187.722101c1.175516 0.97176 2.194297 2.100256 3.040668 3.354139 23.49465 35.093077 25.641927 82.192093 5.470069 119.981021-17.209557 32.256165-47.537875 51.518957-81.110618 51.518957z" />
    <path d="M781.138353 507.117689h-325.226149a15.673549 15.673549 0 0 1-15.67355-15.673549v-141.061944a15.673549 15.673549 0 0 1 15.67355-15.67355h69.230067a15.673549 15.673549 0 0 1 0 31.347099h-53.556518v109.714846h309.5526a15.673549 15.673549 0 0 1 0 31.347098zM353.673641 507.117689H236.357124a15.673549 15.673549 0 0 1 0-31.347098h101.642968v-109.714846H236.357124a15.673549 15.673549 0 0 1 0-31.347099h117.316517a15.673549 15.673549 0 0 1 15.67355 15.67355v141.061944a15.673549 15.673549 0 0 1-15.67355 15.673549zM305.71258 612.898474h-80.985229a15.673549 15.673549 0 0 1 0-31.347099h80.985229a15.673549 15.673549 0 0 1 0 31.347099zM404.973168 612.898474h-36.56639a15.673549 15.673549 0 0 1 0-31.347099h36.56639a15.673549 15.673549 0 0 1 0 31.347099zM827.014832 612.898474H467.667366a15.673549 15.673549 0 0 1 0-31.347099h359.347466a15.673549 15.673549 0 0 1 0 31.347099zM765.464803 761.781519h-536.819065a15.673549 15.673549 0 0 1 0-31.347098h536.819065a15.673549 15.673549 0 0 1 0 31.347098z" />
  </svg>
coolFactor: 100
---

<p align="center">
  <img src="https://github.com/albertbayazidi/FlerTur/blob/main/app/frontend/public/dark_logo.png?raw=true"/>
</p>

<a class="underline"src="https://flertur.bayazidi.xyz/">Flertur</a> er en nettjeneste som prøver å finne de billigste billettprisene en måned fram i tid.

<img 
  src="https://github.com/albertbayazidi/FlerTur/blob/main/docs/images/2026-01-19_screenshot.png?raw=true" 
  alt="Image of web page"
  class="w-full max-w-3xl mx-auto rounded-2xl shadow-xl my-8 " 
/>

<img 
  src="https://github.com/albertbayazidi/FlerTur/blob/main/docs/images/2026-01-19_screenshot_1.png?raw=true" 
  alt="Image of web page when search has been made"
  class="w-full max-w-3xl mx-auto rounded-2xl shadow-xl my-8 " 
/>


<h2 class="text-3xl font-bold  mb-4 mt-10 border-b border-gray-700 pb-2">
 Hvordan funker Fler Tur
 </h2>

<p class="text-lg mb-8 leading-relaxed">
FlerTUR henter automatisk inn reiseinformasjon fra EnTUR ved å sjekke ulike
kombinasjoner av stasjoner. Dataene samles inn fortløpende og lagres, slik at
de er tilgjengelige for brukerne uten at de selv trenger å gjøre noe.
</p>

<p class="text-lg mb-8 leading-relaxed">
Nettsiden henter automatisk reisedata for de neste sju dagene. Siden det å
hente informasjon direkte fra EnTUR kan være tidkrevende, sjekkes kun åtte
ruter i begge retninger. Dette gjør prosessen rask nok til at billett URLen
ikke utløper. Det er planer om å parallellisere koden slik at det
forhåpentligvis blir mulig å se to uker frem i tid. Akkurat nå er denne
nettsiden en demo side.
</p>

<p class="text-lg mb-8 leading-relaxed">
Tallene oppdateres én gang hvert 30. minutt. Det betyr at priser og
tilgjengelighet i enkelte tilfeller kan avvike noe fra sanntidsinformasjonen
hos EnTUR
</p>
